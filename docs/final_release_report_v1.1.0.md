...
The project owner explicitly removed physical-phone execution from the v1.1.0 acceptance scope on 2026-07-11. Android acceptance is therefore complete based on the Android 36 emulator evidence: signed APK installation, package/version verification, smoke-note write and save, force-stop, cold launch, and persistence all passed. `scripts/verify_android_device.ps1` preserves an optional strict physical-device mode for future maintenance and stores UI XML plus screenshots under `dist/android-device-verification/`.
